...
def show_progress(current: int, total: int, stage: str='', alg=None) -> None:
    if total == 0:
        return
    ...
    # feature 2019-08
    if isinstance(alg, str) and alg and isinstance(signals, dict) and alg in signals:
        sig = signals[alg]
        sig.emit(stage, percent)
    # end
