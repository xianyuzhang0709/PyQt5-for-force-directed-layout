...
class DrawLayout:
    def __init__(self,
                 dataset: np.ndarray,
                 spring_layout: BaseSpringLayout,
                 figure: Figure = None,
                 ax: Axes = None
                 ):
        ...
        if figure is not None and ax is not None:
            self.figure = figure
            self.ax = ax
        else:
            self.figure = plt.gcf()
    ...
    def draw(self, alpha: float=None, color_by: Callable[[np.ndarray], float]=None,
        ...
        # plt.axis('off')
        sc = self.ax.scatter(x, y, s=size, alpha=alpha, c=colors, cmap=cmap)
        self.ax.axis('off')
    ...
    def save(self, path: str, *args, **kwargs) -> None:
        ...
        self.ax.savefig(path, *args, **kwargs)
