...
def load_file(name, dtype):
    with open(os.path.join(base_dir, 'datasets', f'{name}'), encoding='utf8') as data_file:
        return np.loadtxt(
            data_file,
            skiprows=1,
            delimiter=',',
...
def load_poker(size):
    return load_file(
        os.path.join('poker', f'poker{size}.csv'),
        np.int16
    )


def read_dataset(fp, dtype=np.int16):
    if not os.path.exists(fp):
        raise FileNotFoundError
    fn, _ = os.path.splitext(fp)
    if _.lower() != '.csv':
        raise ValueError('Only support CSV file')

    with open(fp, encoding='utf8') as data_file:
        return np.loadtxt(
            data_file,
            skiprows=1,
            delimiter=',',
